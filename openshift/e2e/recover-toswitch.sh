...
NODELINK_FILE="${SCRIPT_DIR}/nodelink.json"
...
get_node_ip() {
    ...
    python3 -c "
import json
cfg = json.load(open('${NODELINK_FILE}'))
...
ensure_nic_ip() {
    ...
    if [ "${nic}" = "toswitch1" ]; then
        node_ip=$(get_node_ip "${node}" "ipForKindLeaf")
    elif [ "${nic}" = "toswitch2" ]; then
        node_ip=$(get_node_ip "${node}" "ipForKindLeaf2")
    fi
    [ -z "${node_ip}" ] && return 0
...
echo "recover-toswitch: topology file: ${NODELINK_FILE}"
