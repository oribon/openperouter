...
5. **Assign IPs** — static IPv4+IPv6 on all clab containers and leafkind bridge-facing interfaces
6. **Disable rp_filter on clab** — all FRR containers (spine asymmetric routing)
...
Same fabric as kind (spine, leafA, leafB, hosts) but `toswitch1`/`toswitch2` are `kind: bridge` referencing libvirt bridges instead of clab-managed bridges.
...
| File | Change |
|------|--------|
| `openshift/e2e/setup-clab.sh` | Full OCP clab setup script |
| `openshift/e2e/ocp.clab.yml` | OCP clab topology (bridge references) |
