...
#     --infra-config=../openshift/e2e/topology.json --frrk8s-namespace=openshift-frr-k8s
...
# Assign leafkind IPs on the bridge-facing interfaces (eth2) — IPv4 + IPv6
${CLI} exec clab-kind-leafkind1 ip addr add "${PEERLEAF1_IP}/24" dev eth2
${CLI} exec clab-kind-leafkind1 ip -6 addr add "${PEERLEAF1_IPV6}/64" dev eth2
${CLI} exec clab-kind-leafkind2 ip addr add "${PEERLEAF2_IP}/24" dev eth2
${CLI} exec clab-kind-leafkind2 ip -6 addr add "${PEERLEAF2_IPV6}/64" dev eth2
...
for node in ${NODES}; do
    ...
    NODES_JSON="${NODES_JSON}
    \"${node}\": {\"peerLeafIP\": \"${TS1_V4}\", \"peerLeaf2IP\": \"${TS2_V4}\"}"
done
...
spec:
  asn: 64514
  nics: [toswitch1, toswitch2]
  neighbors:
  - {asn: 64512, address: ${PEERLEAF1_IP}}
  - {asn: 64513, address: ${PEERLEAF2_IP}}
  evpn: {vtepCIDR: 100.65.0.0/24}
EOF
