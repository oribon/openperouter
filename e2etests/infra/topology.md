...
- **nodelink config** — provided per-platform via `--nodelink-config` flag
...
graph TB
    ...
    peerLeaf1 ---|"toswitch1<br/>(nodelink config)"| node0
    peerLeaf1 ---|"toswitch1<br/>(nodelink config)"| node1
    peerLeaf2 ---|"toswitch2<br/>(nodelink config)"| node0
    peerLeaf2 ---|"toswitch2<br/>(nodelink config)"| node1
...
2. **nodelink config** describes the cluster↔fabric connectivity (varies per platform)
3. The test suite reads `nodelink config` to configure BGP peering and neighbor lookups
...
| peerLeaf1 ↔ nodes | nodelink config | varies by platform |
| peerLeaf2 ↔ nodes | nodelink config | varies by platform |
| Underlay CR neighbors | nodelink config | `underlayNeighbors` |
| Underlay CR NICs | nodelink config | `underlayNics` |
